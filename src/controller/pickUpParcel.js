...
const TrackParcel = () => {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API}/createparcel`,
        {
          ...
          headers: {
            ...
            status: "Driver Picked Up Parcel",
          },
        }
      );
  ...
  const handleUpdateStatus = async (parcelId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API}/createparcel/sendemail/${parcelId}`,
        {
          method: "PUT",
          headers: {
          ...
          body: JSON.stringify({ status: "Ready for Pickup" }), // Set the new status here
        }
      );
      ...
      if (json.success) {
        ...
        // Remove the updated parcel from the state
        setParcels((prevParcels) =>
  ...
  const filteredParcels = selectedAddress
    ? parcels.filter((parcel) => parcel.recipient.address === selectedAddress)
    : parcels;
  return (
